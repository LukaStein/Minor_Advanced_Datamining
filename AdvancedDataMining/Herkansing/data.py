...
def confusion(xs, ys, model):
    """Shows 26x26 confusion matrix.

    Keyword arguments:
    xs       -- 144-element lists of pixel values (range 0-1)
    ys       -- 26-element lists of correct digits using one-hot encoding
    ...
    matrix = metrics.confusion_matrix([argmax(y) for y in ys], [argmax(yhat) for yhat in yhats], labels=list(range(26)))
    accuracy = sum(matrix[i][i] for i in range(26)) / len(xs)
